...
const TDRSection = ({
  ...
  return (
    <ContextApi.Consumer>
      {value =>
      <>
        <div className={`vicinity-information personal-details-from ${currentStep === 3 ? `` : `hide-form`}`}>
            <div className="main-title">
                <h3>TDR Section</h3>
            </div>
            <Form className="login-form-type" onSubmit={formik.handleSubmit} autoComplete="off">
                <Form.Group>
                    <Form.Label>{getTranslatedText('label.access_token')}</Form.Label>
                    <Form.Control type="text"  />
                </Form.Group>
                <Form.Group>
                    <Form.Label>{getTranslatedText('label.tdr_cert')}</Form.Label>
                    <Form.Control type="text"  />
                </Form.Group>
